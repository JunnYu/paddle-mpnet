...
def evaluate(model, data_loader, args, global_step):
    ...
    with open(
        f"log/{str(global_step)}_prediction.json", "w", encoding="utf-8"
    ) as writer:
...
def run(args):
    ...
    if args.do_train:
        ...
        train_ds.map(
            partial(prepare_train_features_single, tokenizer=tokenizer, args=args),
            batched=False,
            lazy=False,
        )
        ...
        for epoch in range(num_train_epochs):
            for step, batch in enumerate(train_data_loader):
                ...
                if (
                    global_step % args.save_steps == 0 and global_step >= 15700
