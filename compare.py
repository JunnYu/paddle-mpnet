...
from transformers.models.mpnet.modeling_mpnet import (
    MPNetForMaskedLM as PTMPNetForMaskedLM,
)
import torch
import paddle
...
pd_model = PDMPNetForMaskedLM.from_pretrained("weights/pd/mpnet-base")
pd_model.eval()
pt_model = PTMPNetForMaskedLM.from_pretrained("weights/hg/mpnet-base")
pt_model.eval()

with paddle.no_grad():
    pd_outputs = pd_model(
        paddle.to_tensor(
            [[523, 123, 6123, 523, 5213, 632], [5232, 1231, 6133, 5253, 5555, 6212]]
        )
    )[0]
